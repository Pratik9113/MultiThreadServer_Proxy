...
void checkHTTPversion(char *msg)
{
    int version = -1;
...
void *thread_fn(void *socketNew)
{
    ...
    else if (bytes_send_client > 0)
    {
        ...
        else
        {
            ...
            {
                printf("THis code doesnot support any method from get\n");
            }
        }
    ...
    {
        // request from client does not send
...
cache_element *find(char *url)
{
    ...
    int temp_lock_val = pthread_mutex_locck(&lock);
...
void remove_cache_element()
{
    ...
    if (head == NULL)
    {
        ...
        cache_size = cache_size - (temp->len) - sizeof(cache_element) - strlent(temp->url) - 1;
        free(temp->data);
        free(temp->url);
        free(temp);
    }
